...
def random_and_local(dataset, cycle_time, repeat):

    print("DATASET : " + dataset)
    print("CYCLE TIME : " + str(cycle_time))
    print("REPEAT COUNT : " + str(repeat))

    def random_search(dN, cT, repeat): 
        ...
        if feasibility == False: print("NOT FEASIBLE")
            ...
            while count < task_count * repeat :
                ...
                while len(completed_tasks) != task_count: 
                    ...
                    ##### choosing a random index from available tasks
                    random_assignment_index = random.randint(0,len(available_tasks)-1)
                    ##### assigned task is the task at random index
                    assigned = available_tasks[random_assignment_index]
    ...
    print("RANDOM SEARCH METHOD ***************************** RANDOM SEARCH METHOD ***************************** RANDOM SEARCH METHOD")
    time1 = time.time()
    random_search_result = random_search(dN = dataset, cT = cycle_time, repeat = repeat)
    for i in random_search_result.keys():
        print(f"Station {i} : {random_search_result[i]}")
    print(f"Best number of stations is {len(random_search_result)}.")
    time2 = time.time()
    print("Time Elapsed: " + str(time2-time1) + " seconds")


    print("LOCAL SEARCH WITH RANDOM SEARCH RESULT ***************************** LOCAL SEARCH WITH RANDOM SEARCH RESULT ***************************** LOCAL SEARCH WITH RANDOM SEARCH RESULT")
    time3 = time.time()
    local_best_random = local_search_swap(dN = dataset, cT = cycle_time, best_solution=random_search_result, repeat = repeat)
    for i in local_best_random.keys():
        print(f"Station {i} : {local_best_random[i]}")
    print(f"Best number of stations is {len(local_best_random)}.")
    time4 = time.time()
...
for i in dataset_cycleTime:
    ...
    random_and_local(dataset=i["dataset"], cycle_time=i["cycle_time"], repeat=repeat)
