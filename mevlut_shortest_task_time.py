...
def shortest_and_local(dataset, cycle_time, repeat):

    print("DATASET : " + dataset)
    print("CYCLE TIME : " + str(cycle_time))
    print("REPEAT COUNT : " + str(repeat))

    def shortest_task_time(dN, cT): 
        ...
        if feasibility == False: print("NOT FEASIBLE")
            ...
            while len(completed_tasks) != task_count: 
                ...
                min_task_time = float('inf')

                assigned = None

                for i in available_tasks: 
                    if task_times[i-1] <= min_task_time:
                        assigned = i
                        min_task_time = task_times[i-1]
    ...
    print("SHORTEST TASK TIME METHOD ***************************** SHORTEST TASK TIME METHOD ***************************** SHORTEST TASK TIME METHOD")
    time1 = time.time()
    shortest_task_time_result = shortest_task_time(dN = dataset, cT = cycle_time)
    for i in shortest_task_time_result.keys():
        print(f"Station {i} : {shortest_task_time_result[i]}")
    print(f"Best number of stations is {len(shortest_task_time_result)}.")
    time2 = time.time()
    print("Time Elapsed: " + str(time2-time1) + " seconds")


    print("LOCAL SEARCH WITH SHORTEST T.T. RESULT ***************************** LOCAL SEARCH WITH SHORTEST T.T. RESULT ***************************** LOCAL SEARCH WITH SHORTEST T.T. RESULT")
    time3 = time.time()
    local_best_shortest = local_search_swap(dN = dataset, cT = cycle_time, best_solution=shortest_task_time_result, repeat = repeat)
    for i in local_best_shortest.keys():
        print(f"Station {i} : {local_best_shortest[i]}")
    print(f"Best number of stations is {len(local_best_shortest)}.")
    time4 = time.time()
...
for i in dataset_cycleTime:
    ...
    shortest_and_local(dataset=i["dataset"], cycle_time=i["cycle_time"], repeat=repeat)
